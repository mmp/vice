...
func (s *Sim) ATISCommand(tcw TCW, callsign av.ADSBCallsign, letter string) (speech.CommandIntent, error) {
	...
	return s.dispatchControlledAircraftCommand(tcw, callsign,
		func(tcw TCW, ac *Aircraft) speech.CommandIntent {
			...
			return speech.ATISIntent{Letter: letter}
		})
}
...
func (s *Sim) handleTrafficAdvisory(ac *Aircraft, oclock int, miles, callAlt int, altUnknown bool) speech.CommandIntent {
	...
	logResult := func(intent speech.TrafficAdvisoryIntent) speech.CommandIntent {
		tlg.Info("traffic_advisory", slog.String("result", intent.String()))
		return intent
	}

	if nearestMETAR.ICAO != "" && !nearestMETAR.IsVMC() {
		return logResult(speech.TrafficAdvisoryIntent{Response: speech.TrafficResponseIMC})
	}
	...
	if traffic == nil {
		...
		return logResult(speech.TrafficAdvisoryIntent{Response: speech.TrafficResponseLooking})
	}
	...
	if s.trafficIsVisible(ac, traffic) {
		...
		return logResult(speech.TrafficAdvisoryIntent{
			Response:               speech.TrafficResponseTrafficSeen,
	...
	return s.dispatchAircraftCommand(tcw, callsign,
		...
		func(tcw TCW, ac *Aircraft) speech.CommandIntent {
			if sighting := ac.RecentSighting(); sighting != nil {
				...
				return speech.VisualSeparationIntent{}
			}
			// If they don't have traffic in sight, they can't maintain visual separation
			return speech.MakeUnableIntent("unable, we don't have the traffic")
		})
}
...
func (s *Sim) CautionWakeTurbulence(tcw TCW, callsign av.ADSBCallsign) (speech.CommandIntent, error) {
	...
	return s.dispatchAircraftCommand(tcw, callsign,
		nil,
		func(tcw TCW, ac *Aircraft) speech.CommandIntent {
			return speech.CautionWakeTurbulenceIntent{}
		})
}
...
func (s *Sim) RadarServicesTerminated(tcw TCW, callsign av.ADSBCallsign) (speech.CommandIntent, error) {
	...
	return s.dispatchControlledAircraftCommand(tcw, callsign,
		func(tcw TCW, ac *Aircraft) speech.CommandIntent {
			...
			return speech.ContactIntent{
				Type: speech.ContactRadarTerminated,
			}
		})
}
...
func sayAgainIntent(commandType string) (speech.CommandIntent, error) {
	var cmdType speech.SayAgainCommandType
	switch commandType {
	case "HEADING":
		cmdType = speech.SayAgainHeading
	case "ALTITUDE":
		cmdType = speech.SayAgainAltitude
	case "SPEED":
		cmdType = speech.SayAgainSpeed
	case "APPROACH":
		cmdType = speech.SayAgainApproach
	case "TURN":
		cmdType = speech.SayAgainTurn
	case "SQUAWK":
	...
	return speech.SayAgainIntent{CommandType: cmdType}, nil
}
