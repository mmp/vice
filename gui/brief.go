...
// Brief holds a brief's parse cache and the choices the reader has made in
// it: which option each configuration selector is set to, which headings are
// collapsed, and which controller airspace has been toggled off on each map.
type Brief struct {
	markdown             string
...
// Draw renders the brief for the current sim, resetting the reader's
// choices whenever the brief itself changes.
func (b *Brief) Draw(markdown string, p platform.Platform, baseFontSize int, state *client.SimState,
	controlClient *client.ControlClient, lg *log.Logger) {
	if b.markdown != markdown {
		b.markdown = markdown
		b.selectedConfigs = brief.GetInitialConfigSelections(markdown)
		b.closedHeadings = make(map[string]struct{})
		b.disabledAirspaceTCPs = make(map[string]map[string]bool)
		b.parsed = nil
		b.inlineTokensCache = nil
		if b.videoMapCache == nil {
			b.videoMapCache = newVideoMapCache()
		}
	}

	if b.markdown != "" {
		b.render(b.markdown, p, baseFontSize, state, controlClient, lg)
	}
}

// Invalidate forces the brief to be re-parsed and re-rendered on the next
// Draw; callers use it after reloading the brief's source from disk.
func (b *Brief) Invalidate() { b.markdown = "" }
...
// render draws the brief's markdown with imgui.
func (b *Brief) render(source string, p platform.Platform, baseFontSize int, state *client.SimState,
	controlClient *client.ControlClient, lg *log.Logger) {
	selectedConfigs, closedHeadings, mapCache := b.selectedConfigs, b.closedHeadings, b.videoMapCache
	if state == nil {
		return
	}
	...
	if b.parsed == nil || state.GenerationIndex > b.parseGen {
		b.parsed = brief.ParseMarkdown([]byte(source), brief.ParseOptions{
		...
		b.parseGen = state.GenerationIndex
	...
	if errs := b.parsed.ParseErrors(); len(errs) > 0 {
	...
	preprocessedSource := b.parsed.Source()
	preprocessedNode := b.parsed.AST()
	...
	_ = ast.Walk(preprocessedNode, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
		...
		case *ast.Paragraph:
			if entering {
				...
				b.renderInline(n, preprocessedSource, inlineFonts, wrapWidth)
		...
		case *brief.DocMapNode:
			if entering {
				...
				disabledTCPs := b.disabledAirspaceTCPs[n.Label]
				...
				if airspaceTCPs := collectMapAirspaceTCPs(n.DocMap, state); len(airspaceTCPs) > 1 {
					if disabledTCPs == nil {
						...
						b.disabledAirspaceTCPs[n.Label] = disabledTCPs
					}
...
func findVideoMapsFromLibrary(library *videomaps.Library, mapNames []string) ([]videomaps.STARSMap, []error) {
	if library == nil {
		return nil, nil
	}

	videoMaps := make([]videomaps.STARSMap, 0, len(mapNames))
	var errs []error
...
type videoMapCache struct {
	libraries map[string]*videomaps.Library // file path -> loaded library
...
func newVideoMapCache() *videoMapCache {
	return &videoMapCache{
		libraries: make(map[string]*videomaps.Library),
...
func loadVideoMapLibrary(briefMap *brief.VideoMapBlock, cache *videoMapCache,
	controlClient *client.ControlClient) (*videomaps.Library, error) {
...
// correctly.
func (b *Brief) renderInline(n ast.Node, source []byte, fonts briefInlineFonts, wrapWidth float32) {
	...
	if b.inlineTokensCache == nil {
		b.inlineTokensCache = make(map[ast.Node]inlineRender)
	}
	r, ok := b.inlineTokensCache[n]
	if !ok {
		r = buildInlineRender(n, source, fonts)
		b.inlineTokensCache[n] = r
	}
...
func calculateMapProjection(briefMap *brief.VideoMapBlock, videoMaps []videomaps.STARSMap, state *client.SimState) (briefMapProjection, error) {
...
func drawBriefMap(drawList *imgui.DrawList, briefMap *brief.VideoMapBlock, videoMaps []videomaps.STARSMap,
