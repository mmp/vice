...
import "github.com/mmp/vice/gui"
...
var whatsNew []string = []string{
	...
	"Clicking the " + gui.Icons.Keyboard + " icon on the menubar gives a summary of vice's keyboard commands",
	...
	`Control commands can be entered&mdash;but still not issued&mdash;during pilot readbacks`,
	`Wind/weather simulation is more accurate and based on historical real-world`,
	...
	`Scenario updates: BOS, ACK, MHT (radarcontacto), P50 (Ketan K), COS (Andrew S), BGR (Ryan G), M98 (Logan S)`,
	`More scenario updates: F11, ZJX 50 (Jud Lopez), DAB, F11 (Michael Knight), MIA (Gecko)`,
	...
	`STARS: fixed bug where interfacility handoffs would get a FORMAT error`,
	`ERAM: Added QF and QU commands tearoffs, cursors, and clock`,
	`ERAM: switched tertiary/primary mouse clicks to primary/secondary`,
	`New scenarios: ZJX Consolidated (Jake Magee), ZNY Area B (Ryan G), ZJX 11, 21 (manaphy), ZJX 14, 15 (Michael Knight)`,
	`Scenario updates: ZLA, ZHN (Ketan K), ZNY A, JFK4 finals (Ryan G)`,
	`Updated to latest AIRAC (Michael Knight)`,
	`Sub-windows can now be moved outside the main window; flight strips and messages are now sub-windows`,
	`Text to speech now runs locally on your computer rather than in the cloud`,
	`Added ATIS simulation with speech-to-text support for ATIS updates`,
	`Improved simulation of radio static / distortion`,
	`Added PTT test button (shows strength, plays back as it records)`,
	`ERAM: added support for giving aircraft speeds in mach (Michael Trokel)`,
	`Flight strips reworked; right click to push them to the next controller`,
	`Major rework of how facility-specific data is managed (Michael Trokel)`,
	`New scenarios: LBB (Gecko), ZNY E/F (Ryan G), ZJX 88 (manaphy), TYS (Jake Magee / manaphy), FAT (Ketan K)`,
	`Scenario updates: MIA (Gecko), ZNY, PHL, BOS (Ryan G), BOS, DCA (radarcontacto)`,
	`More scenario updates: I90 (Ketan K), ZJX (manaphy), NCT, SCT, ZLA (Ketan K), N90 (manaphy, Ryan G)`,
	...
	"Flight simulation: Added \"left/right direct {fix}\" instruction (`LDFIX`, `RDFIX` if you're not using STT)",
	`Flight simulation: Improved pilot initial contact messages to include SIDs/STARs, altitude/speed restrictions, assigned headings, etc.`,
	`Flight simulation: Aircraft now continue to descend after reaching the end of a STAR`,
	`STT: "Negative, ..." actually causes the aircraft that misheard the instruction to ignore it.`,
	`STT: "Negative, (updated instructions)" is allowed when an aircraft reads back the wrong instructions (i.e., the callsign isn't required).`,
	`STT: If a VFR calls in with a request, replying "(callsign), XYZ approach" is treated as "go ahead"`,
	`STT: Added support for "expect direct (fix)" / "expect to rejoin the arrival at (fix)"`,
	`STT: Fixed some issues with "at (fix), cleared (approach)"`,
	...
	`ERAM: Added ALTIM SET and WX REPORT floating windows (Michael Knight)`,
	...
	`ERAM: Added support for Nexrad weather radar (Michael Trokel)`,
	`ERAM: Added FDB altitude, speed, and free-form text menus (Michael Trokel)`,
	"ERAM: Added support for point outs (`QP` commands) and quicklook (`QL` commands)",
	"ERAM: Fixed bugs in `LA`/`LB` commands, added `LC` command",
	"ERAM: Added support for setting leader line position and length together: `#/# [FLID]`",
	`ERAM: Added support for pop-up menus to configure more views`,
	`ERAM: Added checklist views`,
	`ERAM: Fixed bug with ERAM cursors not being drawn`,
	`ERAM: Added support for F-key shortcuts (F1 -> QF, etc.)`,
	"STARS: Fixed bug with \"create / modify interfacility VFR flight plan\" command (e.g. `N123AB LAS*SAN C172 165`)",
	...
	`Scenario updates: ZNY, ZAU, C90, Y90 (manaphy), T75 (Ryan H), M98 (Shane, Logan S), ZLA, SCT, ZLA, PCT (Ketan K), F11 (Michael Knight).`,
	`More scenario updates: C90 (Shane, Zach Styka), I90 (Ethan Hawes), EWR (aq86_), OKC (obmna), TPA (Jake), N90, PHL (Matt P), P80 (boganbruce)`,
	...
	`STARS: Fixed bugs with redirected handoff datablocks and recalling redirected handoffs`,
	`STARS: "Outside owned airspace" alerts are no longer shown in datablocks by default. (Can be reenabled in settings.)`,
	`ERAM: Added confict alert detection (Michael Trokel)`,
	`ERAM: Fix leader line alignment with datablocks (Jud Lopez)`,
	`ERAM: Added support for middle-click of callsign in datablock running QF command`,
	"ERAM: Added support for always-displayed maps (must re-run `crc2vice` on source maps)",
	`New scenarios: ILM, HSV (Jake), ZFW (rome, Asian Evxn), ZME (Noah Hunt)`,
	`Scenario updates: PCT SHD, HCF, SCT, L30, NCT (Ketan K), MYR (Jake), I90 (Ethan Hawes), ZME (Noah Hunt)`,
	`More updates: CLT (Gus Agostinho), STL (Ryan H), M98 (Logan S, Shane), ANC, P80, S46 (Mike Fries)`,
	`Even more: N90 (FlyingHigh), TPA (Nelson Tieku), D10 (Seth), A80 (McNugget)`,
	"Flight model: Added: `IFIX/RADIAL`: intercept a fix's radial; also available with voice",
	`Flight model: departures are held when a recent departure crosses their departure path`,
	`Flight model: departures to the same fix are now spaced regardless of their runway`,
	`Flight model: More accurately model departures' 400' AGL rollout`,
	...
	`Fixed bugs with departure sequencing launching multiple a/c to the same exit`,
	`Fixed bugs with aircraft cleared for charted visual approaches not descending`,
	`ERAM: Fixed bug with cutoff "8" characters in datablocks`,
}
