...
func TestSizeOf(t *testing.T) {
	// Test basic types
	var i int64 = 42
	size := SizeOf(i, os.Stdout, false, 0)
	if size != 8 {
	...
	// Test string
	s := "hello"
	size = SizeOf(s, os.Stdout, false, 0)
	...
	// Test slice
	slice := []int{1, 2, 3, 4, 5}
	size = SizeOf(slice, os.Stdout, false, 0)
	...
	size = SizeOf(p, os.Stdout, false, 0)
	...
	size = SizeOf(n1, os.Stdout, false, 0)
...
func TestSizeOfWithPrintMembers(t *testing.T) {
	...
	size := SizeOf(ts, os.Stdout, true, 0)
	if size < 100 {
...
func TestSizeOfWithThreshold(t *testing.T) {
	...
	size := SizeOf(ls, os.Stdout, false, 1000)
	...
	size = SizeOf(c, os.Stdout, false, 2000)
