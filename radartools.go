...
func (c *CRDAConfig) GetGhost(ac *Aircraft) *Aircraft {
	/*
		...
		if !ok {
			...
			h := headingp2ll(ac.TrackPosition(), src.Threshold, sim.MagneticVariation())
...
func (c *CRDAConfig) DrawRegions(ctx *PaneContext, transforms ScopeTransformations, cb *CommandBuffer) {
	...
	rota := src.Heading + 180 - c.GlideslopeLateralSpread - sim.MagneticVariation()
	rotb := src.Heading + 180 + c.GlideslopeLateralSpread - sim.MagneticVariation()
