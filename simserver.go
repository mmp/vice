...
- stars: DEL command to nuke rogue aircraft(?)
...
var jfkConfigs map[string]AirportConfig = map[string]AirportConfig{
	"31s": AirportConfig{},
	"22s": AirportConfig{},
	"13s": AirportConfig{},
	"4s":  AirportConfig{},
...
var configPositions map[string]Point2LL = map[string]Point2LL{
	...
	"_KLGA_HO": Point2LL{parseLatLong("W073.55.01.611"), parseLatLong("N040.46.16.588")},
}
...
func (ssc *SimServerConnectionConfiguration) DrawUI() bool {
	...
	if imgui.BeginComboV("Config", ssc.config, imgui.ComboFlagsHeightLarge) {
		for _, c := range SortedMapKeys(jfkConfigs) {
			if imgui.SelectableV(c, c == ssc.config, 0, imgui.Vec2{}) {
				ssc.config = c
			}
		}
		imgui.EndCombo()
	}
	imgui.InputIntV("Spawn rate (seconds)", &ssc.spawnRate, 15, 120, 0)
	imgui.InputIntV("Spawn rate delta (seconds)", &ssc.spawnRateDelta, -100, 100, 0)
...
func NewSimServer(callsign string, ssc SimServerConnectionConfiguration) *SimServer {
	...
	ss := &SimServer{
		...
		startTime:   time.Now().Add(-30 * time.Second), // hack, part 1 to prime the sim pump...
		simRate:     ssc.simRate,
	}

	// FIXME
	r := float32(ssc.spawnRate)
	d := float32(ssc.spawnRateDelta)
	now := ss.startTime

	switch ssc.config {
	case "31s":
		ss.spawners = append(ss.spawners, NewJFK31LSimSpawner(r, d, now))
		ss.spawners = append(ss.spawners, NewJFK31RSimSpawner(r*3, d, now))

	case "22s":
		ss.spawners = append(ss.spawners, NewJFK22RSimSpawner(r, d, now))

	case "13s":
		ss.spawners = append(ss.spawners, NewJFK13RSimSpawner(r, d, now))

	case "4s":
		ss.spawners = append(ss.spawners, NewJFK4LSimSpawner(r, d, now))

	default:
		lg.Errorf("%s: unknown config!", ssc.config)
	}
	ss.spawners = append(ss.spawners, NewKFRGSimSpawner(r*3, d, now))
	ss.spawners = append(ss.spawners, NewKISPSimSpawner(r*4, d, now))
	ss.spawners = append(ss.spawners, NewKLGASimSpawner(r*3, d, now))
	ss.spawners = append(ss.spawners, NewKLGAPropSimSpawner(r*5, d, now))
	...
	for _, ac := range ss.aircraft {
		...
		if ac.AssignedHeading != nil {
			...
			if ac.TurnDirection != nil {
				turn = 3 * float32(*ac.TurnDirection)
			}
...
func (ss *SimServer) SpawnAircraft(ac *Aircraft, alt int, altAssigned int, speed int) {
	lg.Errorf("%s spawned", ac.Callsign)
...
type SimSpawner struct {
	...
	rate, delta float32 // TODO: various interesting distributions...
...
func (s *SimSpawner) MaybeSpawn(ss *SimServer) {
	...
	seconds := lerp(rand.Float32(), s.rate-s.delta, s.rate+s.delta)
	s.nextSpawn = ss.CurrentTime().Add(time.Duration(seconds * float32(time.Second)))
}
...
func NewJFK31LSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:        now.Add(time.Duration(delta * float32(time.Second))),
	...
	for _, exit := range jfkWater {
		ss.routes = append(ss.routes, Route{
...
func NewJFK31RSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:        now.Add(time.Duration(delta * float32(time.Second))),
		departureAirport: "KJFK",
		rate:             rate,
...
func NewJFK22RSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:        now.Add(time.Duration(delta * float32(time.Second))),
		departureAirport: "KJFK",
		rate:             rate,
	...
	for _, exit := range jfkNorth {
		ss.routes = append(ss.routes, Route{
			Waypoints:       "_JFK_22RL._JFK_4L.#222." + exit[0],
			Scratchpad:      exit[1],
			InitialAltitude: 13,
			ClearedAltitude: 5000,
			InitialSpeed:    0,
...
func NewJFK13RSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:        now.Add(time.Duration(delta * float32(time.Second))),
		departureAirport: "KJFK",
		rate:             rate,
...
func NewJFK4LSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:        now.Add(time.Duration(delta * float32(time.Second))),
		departureAirport: "KJFK",
		rate:             rate,
...
func NewKFRGSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:         now.Add(time.Duration(delta * float32(time.Second))),
...
func NewKISPSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:         now.Add(time.Duration(delta * float32(time.Second))),
	...
	for _, exit := range jfkNorth {
		ss.routes = append(ss.routes, Route{
			Waypoints:       "KISP._KISP_HO.@.#275." + exit[0],
			Scratchpad:      exit[1],
			InitialAltitude: 70,
...
func NewKLGASimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:         now.Add(time.Duration(delta * float32(time.Second))),
	...
	ss.routes = append(ss.routes, Route{
		Waypoints:       "KLGA._KLGA_HO.@.JFK.DIXIE",
		Scratchpad:      "DIX",
		InitialAltitude: 70,
	...
	for i, water := range []string{"SHIPP", "WAVEY", "BETTE"} {
		...
		ss.routes = append(ss.routes, Route{
			Waypoints:       "KLGA._KLGA_HO.@.JFK." + water,
			Scratchpad:      sp[i],
			InitialAltitude: 70,
			ClearedAltitude: 8000,
			InitialSpeed:    0,
...
func NewKLGAPropSimSpawner(rate float32, delta float32, now time.Time) *SimSpawner {
	ss := &SimSpawner{
		nextSpawn:         now.Add(time.Duration(delta * float32(time.Second))),
		departureAirport:  "KLGA",
		rate:              rate,
	...
	ss.routes = append(ss.routes, Route{
		Waypoints:       "KLGA._KLGA_HO.@.JFK.WHITE",
		Scratchpad:      "WHI",
		InitialAltitude: 70,
